...
async function adminLogin() {
    ...
    // --- DEBUG START: Show immediate feedback during login attempt ---
    ...
    if (!email || !password) {
        ...
        adminLoginError.style.color = '#dc3545'; // Red for error
        console.log("Input validation failed: missing email or password.");
        return;
    }

    adminLoginButton.disabled = true;
    // adminLoginError.textContent is already set to 'Attempting login...'
    ...
    try {
        ...
        console.log("Fetching admin status from RTDB...");
        ...
        if (userData && userData.isAdmin) {
            ...
            adminLoginError.textContent = 'Login successful! Loading dashboard...'; // More descriptive success
            adminLoginError.style.color = '#28a745'; // Green for success
            ...
            adminLoginError.textContent = "You are not authorized as an admin. (RTDB check failed)"; // More descriptive error
            adminLoginError.style.color = '#dc3545';
            auth.signOut(); // Force logout if not admin
        }
    } catch (error) {
        console.error("Admin login failed in catch block:", error.message, "Code:", error.code);
        // Provide more detail for common Firebase Auth errors
        ...
        if (error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password') {
            ...
            displayErrorMessage = 'Network error. Please check your internet connection.';
             ...
             displayErrorMessage = 'Invalid email format. (e.g., admin@yourdomain.com)';
        } else if (error.code === 'auth/popup-closed-by-user' || error.code === 'auth/cancelled-popup-request') {
             displayErrorMessage = 'Login cancelled.'; // For popup login methods, not directly applicable here but good to know
        }
        adminLoginError.textContent = displayErrorMessage;
        adminLoginError.style.color = '#dc3545';
    } finally {
        console.log("adminLogin function finished.");
        adminLoginButton.disabled = false; // Re-enable button
    }
}
...
async function createPublicUser() {
    ...
    try {
        // --- CALL YOUR DEPLOYED CLOUD FUNCTION HERE ---
        ...
        // Display user-friendly error from Cloud Function if available
        let errorMessage = error.message;
        if (error.code === 'functions/already-exists') {
            errorMessage = 'A user with that username already exists. Please choose a different one.';
        } else if (error.details) { // Check for custom error details from Cloud Function
             errorMessage = error.details; // Cloud function might send specific details
        }
        createUserMessage.textContent = `Error: ${errorMessage}`;
        createUserMessage.style.color = '#dc3545';
    } finally {
...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        if (userData && userData.isAdmin) {
            ...
            // If the user isn't found as an admin in RTDB, force them to log out
            auth.signOut();
            showAdminLoginSection();
        }
    } else {
